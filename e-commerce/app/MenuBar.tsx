...
{
	return (
		<div className={`w-full bg-[#151515] text-white p-4 overflow-x-auto absolute mt-3 custom-scroll transition duration-300 ${!is_focus ? "opacity-0" : "opacity-100"}`}>
			{products && search.length != 0 && products.filter((value) => (value.title.toLowerCase().includes(search.toLowerCase()))).map((value, index) =>
			{
				return (
					<div key={index} className="flex items-center gap-2 m-1 p-2 rounded-sm bg-[#343232] cursor-pointer hover:bg-[#312f2f]">
...
export default function MenuBar()
{
	...
	let	[products, setProducts] = useState<Products[] | undefined>();
	useEffect(() =>
	{
		async function ftProducts()
		{
			...
			let	get_products: Products[] = await products_api.json();
