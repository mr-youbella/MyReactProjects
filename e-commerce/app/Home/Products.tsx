	...
	return (
		<main id="products">
			<div className="lg:mx-auto lg:w-2/3 bg-linear-to-r from-green-200 to-red-300 rounded-md m-1 relative group">
				...
				<div className="overflow-y-hidden custom-scroll">
					<div className="flex text-[#000000] m-2 gap-2">
						{products && products.filter((value) => (value.is_new_product)).map((value, index) =>
						{
							return (
								<Link key={index} className={slide_product_style.product_div} href={`./Store/${value.title}?id=${value.id}`}>
									...
									<div className="leading-none"> 
										...
										{value.old_price && <del className={slide_product_style.old_product_price}>{value.old_price ? (value.old_price + "DH") : ""}</del>}
									</div>
								</Link>
							);
						})}
			...
			<div className="m-1 lg:mx-auto p-3 lg:w-2/3 rounded-md bg-[#e6e6e6]">
				...
				<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-1 justify-items-center text-black">
